...
Небольшой учебный проект на FastAPI + SQLAlchemy (async) — минимальный API-клон Твиттера:
...
- Настройка базы данных (SQLite / Postgres)
- Запуск приложения
- Тестирование
- Описание эндпоинтов с примерами
- Замечания по безопасности и развёртыванию
- Вклад и лицензия
...
4. Запустить приложение (пример с SQLite file-based):
   - Пример переменной ENGINE:
     export ENGINE="sqlite+aiosqlite:///./data.db"
   - Запуск:
